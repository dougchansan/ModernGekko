...
constexpr std::string_view DOLRECOMP_REVISION =
    "1bec3554ecc4817cf78319ca3d8a0669477f29fa";
...
bool SetActiveModule(const fs::path& output, std::string_view id,
                     const fs::path& module)
{
  const fs::path directory = output / id;
  const fs::path active = directory / "active-module.txt";
  const fs::path pending = directory / "active-module.tmp";
  std::error_code ec;
...

  {
    ...
    file << moderngekko::pgo::PathText(module) << '\n';
    file.close();
    if (!file)
      return false;
  }
...
#if defined(_WIN32)
  if (!MoveFileExW(pending.c_str(), active.c_str(),
                              ...
                              BuildOptions options)
{
  ...
  if (fs::is_regular_file(module))
  {
    if (options.make_active && !SetActiveModule(options.output, game.disc_id, module))
    {
      std::cerr << "could not update active-module.txt\n";
      return std::nullopt;
    }
           ...
           const PgoRunOptions& pgo_options)
{
  ...
  generate_options.make_active = false;
...

  {
    ...
    manifest.close();
    if (!manifest)
      return PgoFailure("manifest write", workspace);
  }

  if (!SetActiveModule(options.output, game.disc_id, *module))
    return PgoFailure("module activation", workspace);
